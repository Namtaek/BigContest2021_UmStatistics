...
본 파일은 2021 빅콘테스트 ECO 제주 참가팀 **UmStatistics**의 마크다운 제출 파일입니다. 이 `Visualization` 파일에서는 주어진 데이터와 외부 데이터를 활용해 PPT에 들어가는 결과물들에 대한 시각화 자료가 포함되어 있습니다.
...
데이터를 통한 시각화는 존재하지 않습니다.

# 2. 데이터 탐색

## 2.1 배출량 데이터 특성 파악

### 2.1.1 행정동별 총 배출량 시각화
...
```{r, fig.align='center', out.width = '90%', message = F}
...
```{r, fig.align='center', out.width = '90%', message = F, warning = F}
daily_trend_data = 
  raw_data %>% filter(emd_nm %in% interest_dong) %>% 
...
## 2.3 배출량 데이터 관측 지점 별 결측치 파악

### 2.3.1 서귀포시 대천동 외 10개 동
...
daily_imputed_data %>% left_join(daily_unimputed_data) %>% filter(emd_nm %notin% outlier_dong) %>% 
  ...
  geom_line(aes(x = base_date, y = em_g, color = 'red'), alpha = 0.8) + 
  geom_line(aes(x = base_date, y = daily_em_g), alpha = 0.8) + 
  facet_wrap(~emd_nm, nrow = 6, scales = 'free') + 
...
monthly_imputed_data %>% left_join(monthly_unimputed_data) %>% filter(emd_nm %in% outlier_dong) %>% 
  ggplot() + 
  geom_line(aes(x = base_date, y = em_g, color = 'red'), size = 1, alpha = 0.8) + 
...
```{r, eval = F, echo = F}
## knitr::purl('4.Visualization.Rmd','4.Visualization.R')
```
